...
class Comment(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    post_id = Column(Integer, ForeignKey("articles.id", ondelete="CASCADE"), index=True)
    user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), index=True)
    content = Column(Text, nullable=False)
    parent_id = Column(
...
class ForumThread(Base):
    ...
    user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), index=True)
    category_id = Column(
        Integer, ForeignKey("forum_categories.id", ondelete="SET NULL"), index=True
    )
    title = Column(String(255), nullable=False)
    content = Column(Text, nullable=False)
...
class ForumReply(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    thread_id = Column(
        Integer, ForeignKey("forum_threads.id", ondelete="CASCADE"), index=True
    )
    user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), index=True)
    content = Column(Text, nullable=False)
    parent_id = Column(
        Integer, ForeignKey("forum_replies.id", ondelete="CASCADE"), index=True
    )
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
...
class Attachment(Base):
    ...
    attached_to_id = Column(Integer, index=True)
