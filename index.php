...
try {
    // Get latest articles
    $latest_articles = $api->request('/articles?skip=0&limit=3');
    
    // Get forum categories
    $categories = $api->request('/forum/categories');
    
    // Get popular threads
    $popular_threads = $api->request('/forum/threads?skip=0&limit=5');
} catch (Exception $e) {
                            ...
                            <?php foreach (array_slice($categories, 0, 2) as $category): ?>
                                <div class="forum-category">
                                    ...
                                    <ul class="forum-topics">
                                        ...
                                        $threads = [];
                                        try {
                                            $threads = $api->request('/forum/threads?category_id=' . $category['id'] . '&limit=2&skip=0');
                                        } catch (Exception $e) {
